...
const MediaSlider = ({ media }: Props) => {
  ...
  useEffect(() => {
    ...
    return () => {
      ws.removeEventListner(mediaPositonResEvent)
    }
  }, [])
  ...
  useEffect(() => {
    ...
    if (media.PlaybackStatus === "Playing" && !interval) {
      ...
      (media.PlaybackStatus === "Paused" || media.PlaybackStatus === "Stopped") &&
      interval
    ) {
  ...
  const seek = (position: number[]) => {
    ...
    ws.emit("mediaSeek", {
      position: position[0].toFixed(0).toString(),
      id: media.Metadata?.trackId,
    })
  }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.textContainer}>
        ...
        <Text style={styles.text}>
          {media.Metadata?.length ? formatTime(+media.Metadata.length) : ""}
        </Text>
      </View>
    </View>
  )
